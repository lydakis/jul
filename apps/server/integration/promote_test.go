...
func TestPromotionRejectsNonFastForward(t *testing.T) {
	repoRoot := t.TempDir()
	repoName := "demo"
	...
	commit1 := strings.TrimSpace(runCmd(t, repo, nil, "git", "rev-parse", "HEAD"))

	writeFile(t, repo, "README.md", "hello\nworld\n")
	runCmd(t, repo, nil, "git", "add", "README.md")
	runCmd(t, repo, nil, "git", "commit", "-m", "feat: second")
	commit2 := strings.TrimSpace(runCmd(t, repo, nil, "git", "rev-parse", "HEAD"))
	...
	refSHA := strings.TrimSpace(runCmd(t, repoRoot, nil, "git", "--git-dir", bareRepo, "rev-parse", "refs/heads/main"))
	if refSHA != commit2 {
		t.Fatalf("expected ref %s, got %s", commit2, refSHA)
	}
}
