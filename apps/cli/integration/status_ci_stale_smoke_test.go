...
func TestStatusHidesStaleCIWithoutConfig(t *testing.T) {
	...
	var status struct {
		DraftCI json.RawMessage `json:"draft_ci"`
	}
	if err := json.NewDecoder(strings.NewReader(statusOut)).Decode(&status); err != nil {
		t.Fatalf("failed to decode status output: %v", err)
	}
	if len(status.DraftCI) != 0 {
		t.Fatalf("expected draft_ci to be hidden when config missing, got %s", string(status.DraftCI))
	}
}
